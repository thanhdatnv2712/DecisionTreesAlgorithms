...
class C45:
	...
	def fetchData(self):
		csv_data = pd.read_csv(self.filePathToData)
		self.classes = list(set(list(csv_data["label"])))
		#add attributes
		for att in csv_data.keys():
			if att != "label":
				self.attrValues[att] = "continuous"
		...
		for attr_index, attribute in enumerate(self.attributes):
			checkDiscrete = self.isAttrDiscrete(self.attributes[attr_index])
			print (attribute, checkDiscrete)
			for data_index, value in enumerate(list(csv_data[attribute])):
				if attr_index == 0:
					self.data.append([])
				if not checkDiscrete:
					value = float(value)
				self.data[data_index].append(value)
		for data_index, value in enumerate(list(csv_data["label"])):
			self.data[data_index].append(value)
	...
	def recursiveGenerateTree(self, curData, curAttributes):
		...
		if len(curData) == 0:
			...
			print (len(remainingAttributes), remainingAttributes, best)
	...
	def isAttrDiscrete(self, attribute):
		...
		elif self.attrValues[attribute] == "continuous":
			return False
		else:
			return True
	...
	def splitAttribute(self, curData, curAttributes):
		...
		for attribute in curAttributes:
			...
			if self.isAttrDiscrete(attribute):
				...
				print (best_attribute,best_threshold,splitted)
		print (best_attribute,best_threshold,splitted)
